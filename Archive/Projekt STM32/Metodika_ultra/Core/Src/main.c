...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
#define usTIM TIM3 // функция привязана к третьему таймеру
/* USER CODE END PTD */
...
/* USER CODE BEGIN PFP */
void usDelay(uint32_t uSec); // Создадим функцию
/* USER CODE END PFP */

/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
const float Sos = 0.034/2; // Объявим константу (Скорость звука)
float distance; // переменная
/* USER CODE END 0 */
...
/* USER CODE BEGIN 4 */
void usDelay(uint32_t uSec) // функция микросекунды
{
	if(uSec < 2) uSec = 2;
	usTIM->ARR = uSec - 1;
	usTIM->EGR = 1;
